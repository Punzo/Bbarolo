...
Spacepar<T>::Spacepar(Cube<T> *c) : Galfit<T>::Galfit(c) {
    ...
    for (int i=0; i<Model::MAXPAR; i++) Galfit<T>::mpar[i]=false;
    ...
    Galfit<T>::maxs[Model::VROT] = 10000;
    Galfit<T>::mins[Model::VROT] = 0;
    Galfit<T>::maxs[Model::VDISP] = 10000;
    Galfit<T>::mins[Model::VDISP] = 0;
    Galfit<T>::maxs[Model::Z0] = 10000;
    Galfit<T>::mins[Model::Z0] = 0;
    Galfit<T>::maxs[Model::INC] = 90;
    Galfit<T>::mins[Model::INC] = 0;
    Galfit<T>::maxs[Model::PA] = 360;
    Galfit<T>::mins[Model::PA] = 0;
    Galfit<T>::maxs[Model::XPOS] = Galfit<T>::in->DimX()-1;
    Galfit<T>::mins[Model::XPOS] = 0;
    Galfit<T>::maxs[Model::YPOS] = Galfit<T>::in->DimY()-1;
    Galfit<T>::mins[Model::YPOS] = 0;
    Galfit<T>::maxs[Model::VRAD] = 10000;
    Galfit<T>::mins[Model::VRAD] = -10000;

    std::string cunit2 = Galfit<T>::in->Head().Cunit(2);
    Galfit<T>::maxs[Model::VSYS] = AlltoVel(Galfit<T>::in->getZphys(Galfit<T>::in->DimZ()-1), Galfit<T>::in->Head());
    Galfit<T>::mins[Model::VSYS] = AlltoVel(Galfit<T>::in->getZphys(0), Galfit<T>::in->Head());
...
#pragma omp for 
        ...
        for (int i=0; i<p1_nsteps; i++) {
            ...
            for (int j=0; j<p2_nsteps; j++) {
                ...
                T par[Model::MAXPAR];
                par[Model::VROT] = dring->vrot.back();
                par[Model::VDISP] = dring->vdisp.back();
                par[Model::DENS] = dring->dens.back();
                par[Model::Z0] = dring->z0.back();
                par[Model::INC] = dring->inc.back();
                par[Model::PA] = dring->phi.back();
                par[Model::XPOS] = dring->xpos.back();
                par[Model::YPOS] = dring->ypos.back();
                par[Model::VSYS] = dring->vsys.back();
                par[Model::VRAD] = dring->vrad.back();
